...
class NetFactory():
    def __init__(self, noise, printAll = True):
        if torch.cuda.is_available():
            ...
            self.net = Convolutional_Neural_Net()
            
        self.printAll = printAll
        self.comment = ""
        if noise == "low":
            if not (os.path.isfile("lowValidationNoise.pt")):
                noiseGen =  torch.distributions.normal.Normal(0.0, signalRMS*noiseDict[noise], validate_args=None)
                lowValidationNoise = noiseGen.sample(fluxValidate.shape)
                torch.save(lowValidationNoise, "lowValidationNoise.pt")
                self.fluxValidate = fluxValidate + lowValidationNoise
            else:
                self.fluxValidate = fluxValidate + torch.load("lowValidationNoise.pt")
        elif noise == "mid":
            if not (os.path.isfile("midValidationNoise.pt")):
                noiseGen =  torch.distributions.normal.Normal(0.0, signalRMS*noiseDict[noise], validate_args=None)
                midValidationNoise = noiseGen.sample(fluxValidate.shape)
                torch.save(midValidationNoise, "midValidationNoise.pt")
                self.fluxValidate = fluxValidate + midValidationNoise
            else:
                self.fluxValidate = fluxValidate + torch.load("midValidationNoise.pt")
        elif noise == "high":
            if not (os.path.isfile("highValidationNoise.pt")):
                noiseGen =  torch.distributions.normal.Normal(0.0, signalRMS*noiseDict[noise], validate_args=None)
                highValidationNoise = noiseGen.sample(fluxValidate.shape)
                torch.save(highValidationNoise, "highValidationNoise.pt")
                self.fluxValidate = fluxValidate + highValidationNoise
            else:
    ...
    def randomSample(self, num, noise="low", source="train"):
        ...
        if torch.cuda.is_available():
            ...
            scatterPoints = torch.zeros(num, 512, dtype=torch.long)
